"""
Enhanced Multimodal Website Content Extractor Tool with Interactive Interface

This tool allows users to:
1. Input website addresses to extract multimodal content (text, images, structure)
2. View previously extracted documents
3. Save extracted content
4. Delete stored documents
"""
...
class DocumentManager:
    """Manages storage and retrieval of extracted documents"""

    def __init__(self, storage_dir="extracted_docs"):
    ...
    def add_document(self, doc_id, url, title, content, timestamp=None):
...
class InteractiveExtractor:
    def __init__(self):
        ...
        self.multimodal_doc_manager = MultimodalDocumentManager()

    def display_menu(self):
        """Display the main menu"""
        print("\n" + "="*60)
        print("MULTIMODAL WEBSITE CONTENT EXTRACTOR TOOL")
        print("="*60)
        print("1. Extract text-only content from a website")
        print("2. Extract multimodal content from a website (text + images + structure)")
        print("3. View extracted documents")
        print("4. View specific document")
        print("5. Delete a document")
        print("6. Save document to external file")
        print("7. View multimodal documents")
        print("8. View specific multimodal document")
        print("9. Delete multimodal document")
        print("10. Exit")
        print("-"*60)
    ...
    def extract_text_only(self):
        """Handle text-only website extraction"""
        print("\n--- TEXT-ONLY WEBSITE EXTRACTION ---")
        url = input("Enter the website URL to extract (e.g., https://example.com): ").strip()
        ...
        try:
            max_pages_input = input("Enter maximum number of pages to scan (default 20): ").strip()
            max_pages = int(max_pages_input) if max_pages_input else 20
        except ValueError:
            max_pages = 20
            print("Invalid input. Using default value of 20 pages.")
        ...
        try:
            ...
            delay = 1.0
            print("Invalid input. Using default value of 1 second.")
        ...
        # Get a title for the document (use domain name if no title found)
        ...
        # Store the document
        self.doc_manager.add_document(doc_id, url, title, content)

        print(f"\nText extraction complete!")
        print(f"Document ID: {doc_id}")
        print(f"Title: {title}")
        print(f"Pages processed: {len(extractor.text_content)}")
        print(f"Content saved to document manager.")

    def extract_multimodal(self):
        """Handle multimodal website extraction"""
        print("\n--- MULTIMODAL WEBSITE EXTRACTION ---")
        url = input("Enter the website URL to extract (e.g., https://example.com): ").strip()
        ...
        try:
            max_pages_input = input("Enter maximum number of pages to scan (default 10): ").strip()
        ...
        save_images_input = input("Save images? (yes/no, default yes): ").strip().lower()
        save_images = save_images_input in ['', 'yes', 'y']

        print(f"\nStarting multimodal extraction from {url}...")
        extractor = MultimodalWebsiteExtractor(url, max_pages, delay, save_images)
        ...
        print(f"Content saved to multimodal document manager.")

    def view_documents(self):
        """View all extracted documents"""
        print("\n--- ALL EXTRACTED TEXT DOCUMENTS ---")
        docs = self.doc_manager.list_documents()

        if not docs:
            print("No text documents found.")
        else:
            print(f"{'ID':<15} {'Title':<30} {'Date':<20}")
            print("-" * 70)

            for doc in docs:
                # Truncate title if too long
                title = doc['title'][:27] + "..." if len(doc['title']) > 30 else doc['title']
                date = doc['timestamp'][:19]  # YYYY-MM-DDTHH:MM:SS
                print(f"{doc['id']:<15} {title:<30} {date:<20}")

    def view_multimodal_documents(self):
        """View all multimodal documents"""
        print("\n--- ALL MULTIMODAL DOCUMENTS ---")
        docs = self.multimodal_doc_manager.list_documents()

        if not docs:
            print("No multimodal documents found.")
        else:
            print(f"{'ID':<20} {'Title':<30} {'Date':<20} {'Images':<10}")
            print("-" * 85)
            ...
            for doc in docs:
                ...
                title = doc['title'][:27] + "..." if len(doc['title']) > 30 else doc['title']
                date = doc['timestamp'][:19]  # YYYY-MM-DDTHH:MM:SS
                print(f"{doc['id']:<20} {title:<30} {date:<20} {doc['image_count']:<10}")

    def view_specific_document(self):
        """View a specific document"""
        print("\n--- VIEW SPECIFIC TEXT DOCUMENT ---")
        doc_id = input("Enter document ID to view: ").strip()

        doc = self.doc_manager.get_document(doc_id)
        if not doc:
            print(f"No text document found with ID: {doc_id}")
            return
    ...
    def view_specific_multimodal_document(self):
        """View a specific multimodal document"""
        print("\n--- VIEW SPECIFIC MULTIMODAL DOCUMENT ---")
        doc_id = input("Enter multimodal document ID to view: ").strip()

        doc = self.multimodal_doc_manager.get_document(doc_id)
        if not doc:
            print(f"No multimodal document found with ID: {doc_id}")
            return
        ...
        print(f"Title: {doc['title']}")
        print(f"Timestamp: {doc['timestamp']}")
        print(f"Images included: {doc['image_count']}")
        print(f"Image files: {doc['image_files'][:5]}{'...' if len(doc['image_files']) > 5 else ''}")
        print("\nContent:")
        print("="*80)
        print(doc['content'])
        print("="*80)
    ...
    def delete_document(self):
        ...
        confirm = input("\nAre you sure you want to delete this document? (yes/no): ").strip().lower()
        if confirm in ['yes', 'y']:
    ...
    def delete_multimodal_document(self):
        """Delete a multimodal document"""
        print("\n--- DELETE MULTIMODAL DOCUMENT ---")
        doc_id = input("Enter multimodal document ID to delete: ").strip()
        ...
        confirm = input("\nAre you sure you want to delete this multimodal document? (yes/no): ").strip().lower()
        if confirm in ['yes', 'y']:
            if self.multimodal_doc_manager.delete_document(doc_id):
                print(f"Multimodal document {doc_id} deleted successfully.")
            else:
                print(f"Failed to delete multimodal document {doc_id}.")
        else:
            print("Deletion cancelled.")

    def save_external_file(self):
        """Save a document to an external file"""
        print("\n--- SAVE DOCUMENT TO EXTERNAL FILE ---")
        doc_type = input("Save text document (t) or multimodal document (m)? (t/m): ").strip().lower()

        if doc_type == 't':
            doc_id = input("Enter text document ID to save: ").strip()
            doc = self.doc_manager.get_document(doc_id)
            if not doc:
                print(f"No text document found with ID: {doc_id}")
                return
        elif doc_type == 'm':
            doc_id = input("Enter multimodal document ID to save: ").strip()
            doc = self.multimodal_doc_manager.get_document(doc_id)
            if not doc:
                print(f"No multimodal document found with ID: {doc_id}")
                return
        else:
            print("Invalid choice. Please enter 't' for text or 'm' for multimodal.")
            return

        filename = input(f"Enter filename to save as (default: {doc_id}.txt): ").strip()
        if not filename:
    ...
    def run(self):
        ...
        print("Welcome to the Multimodal Website Content Extractor Tool!")
        ...
        while True:
            ...
            if choice == 1:
                ...
                self.extract_multimodal()
            elif choice == 3:
                self.view_documents()
            elif choice == 4:
                self.view_specific_document()
            elif choice == 5:
                self.delete_document()
            elif choice == 6:
                self.save_external_file()
            elif choice == 7:
                self.view_multimodal_documents()
            elif choice == 8:
                self.view_specific_multimodal_document()
            elif choice == 9:
                self.delete_multimodal_document()
            elif choice == 10:
                print("\nThank you for using the Multimodal Website Content Extractor Tool!")
                break

            input("\nPress Enter to continue...")


# Import the original WebsiteTextExtractor class from the old file
class WebsiteTextExtractor:
    ...
    def generate_document_content(self):
        """Generate a structured document from extracted text"""
        content = f"Website Text Extraction Report\n"
        content += f"Base URL: {self.base_url}\n"
        content += f"Extraction Date: {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
        content += f"Number of Pages Processed: {len(self.text_content)}\n"
        content += "="*80 + "\n\n"

        for url, data in self.text_content.items():
            content += f"URL: {url}\n"
            content += "-"*80 + "\n"
            ...
            if data['articles'] or data['sections'] or data['divs']:
...
def main():
    ...
    # Create and run the interactive extractor
    app = InteractiveExtractor()
    app.run()
